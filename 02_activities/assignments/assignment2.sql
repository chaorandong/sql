...
SELECT 
COALESCE (product_name || ', ' || product_size|| ' (' || product_qty_type || ')',
...
DROP TABLE IF EXISTS total_sale_by_date;
DROP TABLE IF EXISTS best_sale_date;
DROP TABLE IF EXISTS worst_sale_date;

CREATE TEMP TABLE total_sale_by_date AS -- Create the temp table to find total values by market date
SELECT market_date, 
SUM(quantity * cost_to_customer_per_qty) AS total_sale
FROM customer_purchases
GROUP BY market_date;

CREATE TEMP TABLE best_sale_date AS
SELECT *
FROM (
			SELECT *,
			RANK() OVER (ORDER BY total_sale DESC) as best_sale
			FROM total_sale_by_date
) x
WHERE x.best_sale = 1;

CREATE TEMP TABLE worst_sale_date AS
SELECT *
FROM (
			SELECT *,
			RANK() OVER (ORDER BY total_sale ASC) as worst_sale
			FROM total_sale_by_date
) x
WHERE x.worst_sale = 1;
...
x.vendor_name,
x.product_name,
SUM(x.gain_per_customer) AS total_gain_per_product
FROM (
					SELECT DISTINCT
					v.vendor_name, 
					p.product_name,
					vi.original_price*5 AS gain_per_customer,
					c.customer_id
...
VALUES (24, 'Cookie', 'large', 2, 'unit',CURRENT_TIMESTAMP);
